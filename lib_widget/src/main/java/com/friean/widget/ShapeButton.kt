...
class ShapeButton @JvmOverloads constructor(
    ...
    init {
        context.obtainStyledAttributes(attrs, R.styleable.ShapeButton).apply {
            mShapeMode = getInt(R.styleable.ShapeButton_sp_shapeMode, 0)
            mFillColor = getColor(R.styleable.ShapeButton_sp_fillColor, 0xFFFFFFFF.toInt())
            mPressedColor =
                getColor(R.styleable.ShapeButton_sp_pressedColor, 0xFF666666.toInt())
            mStrokeColor = getColor(R.styleable.ShapeButton_sp_strokeColor, 0)
            mStrokeWidth = getDimensionPixelSize(R.styleable.ShapeButton_sp_strokeWidth, 0)
            mCornerRadius = getDimensionPixelSize(R.styleable.ShapeButton_sp_cornerRadius, 0)
            mCornerPosition = getInt(R.styleable.ShapeButton_sp_cornerPosition, -1)
            mActiveEnable = getBoolean(R.styleable.ShapeButton_sp_activeEnable, false)
            mDrawablePosition = getInt(R.styleable.ShapeButton_sp_drawablePosition, -1)
            mStartColor = getColor(R.styleable.ShapeButton_sp_startColor, 0xFFFFFFFF.toInt())
            mEndColor = getColor(R.styleable.ShapeButton_sp_endColor, 0xFFFFFFFF.toInt())
            mOrientation = getColor(R.styleable.ShapeButton_sp_orientation, 0)
            recycle()
        }
    }
