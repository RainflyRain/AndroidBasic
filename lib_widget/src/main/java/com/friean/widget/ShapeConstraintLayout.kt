...
class ShapeConstraintLayout @JvmOverloads constructor(
    ...
    init {
        context.obtainStyledAttributes(attrs, R.styleable.ShapeConstraintLayout).apply {
            mShapeMode = getInt(R.styleable.ShapeConstraintLayout_sp_shapeMode, 0)
            mFillColor = getColor(R.styleable.ShapeConstraintLayout_sp_fillColor, 0xFFFFFFFF.toInt())
            mStrokeColor = getColor(R.styleable.ShapeConstraintLayout_sp_strokeColor, 0)
            mStrokeWidth = getDimensionPixelSize(R.styleable.ShapeConstraintLayout_sp_strokeWidth, 0)
            mCornerRadius = getDimensionPixelSize(R.styleable.ShapeConstraintLayout_sp_cornerRadius, 0)
            mCornerPosition = getInt(R.styleable.ShapeConstraintLayout_sp_cornerPosition, -1)
            mStartColor = getColor(R.styleable.ShapeConstraintLayout_sp_startColor, 0xFFFFFFFF.toInt())
            mEndColor = getColor(R.styleable.ShapeConstraintLayout_sp_endColor, 0xFFFFFFFF.toInt())
            mOrientation = getColor(R.styleable.ShapeConstraintLayout_sp_orientation, 0)
            mWithElevation = getBoolean(R.styleable.ShapeConstraintLayout_sp_withElevation, false)
            recycle()
        }
    }
